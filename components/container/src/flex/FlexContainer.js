...
export default class FlexBoxContainer extends BaseContainer {
  mounted () {
    if (this.el) {
      this.el.addEventListener('click', this.doOnClick.bind(this))
    }
  }
  ...
  destroy () {
    if (this.el) {
      this.el.removeEventListener('click', this.doOnClick.bind(this))
    }
  }
  ...
  getChildStyle (node) {
    ...
    if (configStyle.margin) {
      style.margin = configStyle.margin
    } else {
      style.margin = ''
    }
    if (configStyle.flex) {
    ...
    if (configStyle.alignSelfStretch) { // 优先定义了交叉拉伸 则设置alignSelf
